...
const Dashboard: React.FC<DashboardProps> = ({ parties, tasks }) => {
  // Stats Calculation
  const upcomingPartiesCount = parties.filter(p => safeDate(p.date) >= new Date(new Date().setHours(0,0,0,0))).length;
  ...
  const nextParty = parties
    .filter(p => safeDate(p.date) >= new Date(new Date().setHours(0,0,0,0)))
    .sort((a, b) => safeDate(a.date).getTime() - safeDate(b.date).getTime())[0];
      ...
      {nextParty ? (
        <div className="bg-gradient-to-r from-primary to-rose-500 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <div className="opacity-90 text-base font-medium">{formatDate(nextParty.date)} às {nextParty.time}</div>
